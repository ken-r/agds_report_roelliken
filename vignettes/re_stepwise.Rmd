...
half_hourly_fluxes |>  ggplot(aes(x = PPFD_IN, y = GPP_NT_VUT_REF)) +
  ...
  geom_smooth(formula = y ~ x, method = "lm", aes(color = "lm"), 
              se = FALSE, na.rm = TRUE) +
  labs(x = expression(paste("PPFD IN (", mu, "mol Photon m"^-2, "s"^-1, ")")), 
...
plot <- optimal_metrics |> ggplot(aes(x = predictor_number, y = R2, group = 1)) +
  ...
  labs(x = "p", y = expression(R^2),
       title = expression(paste(R^2, " after p steps"))
  )
plot
...
plot <- optimal_metrics |> ggplot(aes(x = predictor_number, y = AIC, group = 1)) +
  ...
  geom_line() +
  labs(x = "p", y = "AIC",
       title = "AIC after p steps")
plot
```

They both develop similarly over time, just in opposite directions (because for $\text{AIC}$ lower means better and for $R^2$ higher means better). Until $p = 4$ there was a high increase (decrease) with every added predictor, then the increase (decrease) then it flattened.
