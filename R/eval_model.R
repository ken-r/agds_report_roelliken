...
eval_model <- function(mod, df_train, df_test){
  ...
  plot_1 <- ggplot2::ggplot(data = df_train, aes(GPP_NT_VUT_REF, fitted)) +
    ggplot2::geom_point(alpha = 0.3) +
    ggplot2::geom_smooth(method = "lm", se = FALSE, color = "red") +
    ggplot2::geom_abline(slope = 1, intercept = 0, linetype = "dotted") +
    ggplot2::labs(subtitle = bquote( italic(R)^2 == .(format(rsq_train, digits = 2)) ~~
                              RMSE == .(format(rmse_train, digits = 3))),
         title = "Training set") +
    theme_classic()
  
  plot_2 <- ggplot2::ggplot(data = df_test, aes(GPP_NT_VUT_REF, fitted)) +
    ggplot2::geom_point(alpha = 0.3) +
    ggplot2::geom_smooth(method = "lm", se = FALSE, color = "red") +
    ggplot2::geom_abline(slope = 1, intercept = 0, linetype = "dotted") +
    ggplot2::labs(subtitle = bquote( italic(R)^2 == .(format(rsq_test, digits = 2)) ~~
